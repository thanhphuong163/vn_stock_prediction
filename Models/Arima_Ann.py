...
class AnnModel:
	def __init__(self, historical_data: pd.Series, lag=5, hidden_layers=(5,)):
		...
		self.validate_model = MLPRegressor(solver='adam', alpha=1e-5,
		                          hidden_layer_sizes=hidden_layers,
		                          activation='tanh', random_state=None)
	...
	def validate(self, test_data):
		...
		for i in range(len(test_data)):
			# Initialize
			data = np.reshape(historical_data.values, (len(historical_data), 1))
			# Transform data
			supervised_data = self.transform_to_supervised_data(data, index=historical_data.index)
			# Fit model
			y = supervised_data['lag_0']
			x = supervised_data.drop('lag_0', axis=1)
			self.validate_model.fit(x, y)
			# Get prediction
			data_ = historical_data[-self.lag:].append(test_data)
			reshaped_data_ = np.reshape(data_.values, (len(data_), 1))
			supervised_data_ = self.transform_to_supervised_data(reshaped_data_, index=data_.index)
			x_ = supervised_data_.drop('lag_0', axis=1)
			pred = self.validate_model.predict(x_)
			predictions.append(pred[0])
			historical_data = historical_data.append(test_data[i:i+1])
		reshaped_pred = np.reshape(predictions, (len(predictions), 1))
		prediction = self.scaler.inverse_transform(reshaped_pred)
		return self.historical_data[-1:].append(pd.Series(data=prediction.flatten(), index=test_data.index))
